...
export function GamesMenu({
  ...
  useEffect(() => {
    getHighScores("article-mania")
      .then((r) => setScores(r.entries))
      .catch(() => setScores([]));
  }, []);
...
function ScoreRow({
  ...
  return (
    ...
    >
      ...
      <span class="w-12 shrink-0 text-right font-semibold tabular-nums">{entry.score}%</span>
    </div>
  );
}
        ...
        <div class="mt-8 grid grid-cols-3 gap-2.5">
          {GERMAN_ARTICLES.map((a) => (
            <button
              key={a}
              onClick={() => pick(a)}
              class={`rounded-xl border px-3 py-4 text-lg font-medium transition-colors ${articleClass(
                a,
                picked,
                current.article,
              )}`}
...
function articleClass(
  ...
  picked: GermanArticle | null,
  correct: GermanArticle,
): string {
  if (picked === null) return "border-slate-200 bg-white text-slate-800 hover:bg-slate-50";
  if (article === correct) return "border-green-500 bg-green-50 text-green-900";
  if (article === picked) return "border-red-400 bg-red-50 text-red-900";
